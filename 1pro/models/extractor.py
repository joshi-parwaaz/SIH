...
# Import the new reverse image checker
try:
    from models.reverse_image_checker import check_text_with_images
    REVERSE_IMAGE_AVAILABLE = True and not FAST_MODE  # Disable in fast mode
except ImportError:
    logger.warning("Reverse image checker not available")
...
def detect_misinformation(text: str) -> bool:
    ...
    if REVERSE_IMAGE_AVAILABLE and not FAST_MODE:
        try:
            image_analysis = check_text_with_images(text)
