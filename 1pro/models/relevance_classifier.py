...
# Check if we're in fast mode (for development)
FAST_MODE = os.getenv('FAST_MODE', '0') == '1'

try:
    from transformers import pipeline
    TRANSFORMERS_AVAILABLE = True and not FAST_MODE  # Disable in fast mode
except ImportError:
...
def _get_classifier():
    ...
    if _classifier is None:
        try:
            # Use smaller, faster model instead of the huge xlm-roberta-large
            # This model is only ~100MB vs 1GB+ for xlm-roberta-large
            _classifier = pipeline(
                "zero-shot-classification", 
                model="facebook/bart-large-mnli"  # Much faster English model
            )
            logger.info("Initialized fast relevance classifier")
        except Exception as e:
            logger.error(f"Failed to load classifier: {e}")
            # If even BART fails, disable ML classification
...
def is_relevant(text: str) -> bool:
    ...
    """
    if FAST_MODE:
        # In fast mode, always use keyword-based classification
        logger.debug("Fast mode: using keyword-based classification")
        return is_relevant_keywords(text)
