...
class AnomalyData(BaseModel):
    ...
    anomalies_by_type: Dict[str, List[Dict[str, Any]]] = Field(..., description="Anomalies grouped by type")
    all_anomalies: List[Dict[str, Any]] = Field(..., description="All detected anomalies")
    analysis_timestamp: str = Field(..., description="When analysis was performed")
    alert_level: str = Field(..., description="Overall alert level (normal, moderate, elevated, critical)")


class VisualizationData(BaseModel):
    """Visualization data for maps and dashboards."""
    
    metadata: Dict[str, Any] = Field(..., description="Visualization metadata")
    markers: List[Dict[str, Any]] = Field(..., description="Map markers with coordinates")
    heat_map_data: List[Dict[str, Any]] = Field(..., description="Heatmap data points")
    clusters: List[Dict[str, Any]] = Field(..., description="Clustered hazard data")
    legend: Dict[str, Any] = Field(..., description="Legend information for visualization")
    layer_options: Dict[str, Any] = Field(..., description="Layer display options")
    anomaly_overlays: Optional[List[Dict[str, Any]]] = Field(default=None, description="Anomaly visualization overlays")
...
class PipelineResponse(BaseModel):
    ...
    anomaly_analysis: Optional[AnomalyData] = Field(default=None, description="Anomaly detection results")
    visualization_data: Optional[VisualizationData] = Field(default=None, description="Visualization data for maps")
