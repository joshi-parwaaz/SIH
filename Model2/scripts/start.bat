...
if not exist .env (
    ...
    (
        ...
        echo API_PORT=8000
        echo.
        echo # Database Configuration (CHANGE THESE PASSWORDS!^)
        echo MONGO_ROOT_USERNAME=admin
        echo MONGO_ROOT_PASSWORD=your_secure_mongo_password_here
        echo MONGO_DATABASE=ocean_hazard_db
        echo REDIS_PASSWORD=your_secure_redis_password_here
        echo MONGODB_URI=mongodb://admin:%%MONGO_ROOT_PASSWORD%%@mongo:27017/ocean_hazard_db?authSource=admin
        echo REDIS_URL=redis://:%%REDIS_PASSWORD%%@redis:6379/0
        echo.
        echo # Service Passwords (CHANGE THESE!^)
        echo GRAFANA_PASSWORD=your_secure_grafana_password_here
        echo JUPYTER_TOKEN=your_secure_jupyter_token_here
        echo.
