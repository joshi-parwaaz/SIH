...
Now includes reverse image lookup for enhanced misinformation detection.
"""
...
# Import the enhanced reverse image checker
try:
    from models.enhanced_reverse_image_checker import EnhancedReverseImageChecker
    enhanced_checker = EnhancedReverseImageChecker()
    REVERSE_IMAGE_AVAILABLE = True and not FAST_MODE  # Disable in fast mode
except ImportError:
    logger.warning("Enhanced reverse image checker not available")
...
def detect_misinformation(text: str) -> bool:
    """
    Enhanced misinformation detection with reverse image lookup.
    
    Args:
        text (str): Input text (may contain image URLs)
    ...
    text_misinformation = False
    for pattern in misinfo_patterns:
        if re.search(pattern, text_lower):
            text_misinformation = True
            break
    ...
    # Enhanced: Check for images if available (skip in fast mode)
    image_misinformation = False
    if REVERSE_IMAGE_AVAILABLE and not FAST_MODE:
        try:
            # Use enhanced image checker
            image_analysis = enhanced_checker.verify_content_authenticity(text, "unknown")
            image_misinformation = image_analysis.get("is_likely_misinformation", False)
            
            if image_misinformation:
                logger.info(f"Image-based misinformation detected in text: {text[:100]}...")
                
        except Exception as e:
            logger.warning(f"Image analysis failed: {e}")
    elif FAST_MODE:
        logger.debug("Fast mode: skipping image analysis")
    
    # Return True if either text or image analysis indicates misinformation
    return text_misinformation or image_misinformation
